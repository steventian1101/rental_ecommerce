...
const CreateItem = () => {
    ...
    useEffect(() => {
        ...
        if (sideBar == 1) {
            let temp = [];
            temp.push(<AddItemImg setSideBar={setSideBar} setProfileImgs={setProfileImgs} profileImgs={profileImgs} />);
            setDrawSidebar(temp);
        }
        if (sideBar == 2) {
            let temp = [];
            temp.push(<AddItemInfo setSideBar={setSideBar} setItemInfo={setItemInfo} itemInfo={itemInfo} />);
            setDrawSidebar(temp);
        }
        if (sideBar == 3) {
            let temp = [];
            temp.push(<AddChargeRate setSideBar={setSideBar} setChargeRate={setChargeRate} chargeRate={chargeRate} />);
            setDrawSidebar(temp);
        }
    }, [sideBar]);
    useEffect(() => {
    ...
    useEffect(() => {
        profileImgs && profileImgs && console.log(profileImgs)
    }, [profileImgs.length])
    return (
