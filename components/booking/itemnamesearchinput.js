...
const ItemNameSearchInput = ({setId}) => {
    ...
    return (<>
       ...
       {displayalgoliaresult? <div className="overflow-auto  max-h-40" onScroll={handleScroll} style={{ background:"#ffffff1a"}}>
            {
                userCredential.email && hits.map((hit, index) =>(
                     <p className="overflow-hidden instantitemname" key={index} onClick={()=>handleName(hit)}>{hit["item_name"]}</p>
                ))
            }
        </div>:<></>}
        </>
